...
class NumberDialog(ViewBase):
	...
	def __init__(self, model, label_text, unit_text, initial='0', accept=None,
		...
		self.buttons = ui.ButtonGrid(model.width, model.height, 4, 5)
		self.buttons.add(0, 1, '1', font_size=spektrum.NUM_FONT, click=self.number_click)
		self.buttons.add(1, 1, '2', font_size=spektrum.NUM_FONT, click=self.number_click)
		self.buttons.add(2, 1, '3', font_size=spektrum.NUM_FONT, click=self.number_click)
		self.buttons.add(0, 2, '4', font_size=spektrum.NUM_FONT, click=self.number_click)
		self.buttons.add(1, 2, '5', font_size=spektrum.NUM_FONT, click=self.number_click)
		self.buttons.add(2, 2, '6', font_size=spektrum.NUM_FONT, click=self.number_click)
		self.buttons.add(0, 3, '7', font_size=spektrum.NUM_FONT, click=self.number_click)
		self.buttons.add(1, 3, '8', font_size=spektrum.NUM_FONT, click=self.number_click)
		self.buttons.add(2, 3, '9', font_size=spektrum.NUM_FONT, click=self.number_click)
		self.buttons.add(1, 4, '0', font_size=spektrum.NUM_FONT, click=self.number_click)
		self.buttons.add(2, 4, '.', font_size=spektrum.NUM_FONT, click=self.decimal_click)
		self.buttons.add(0, 4, 'HAPUS', click=self.delete_click)
		...
		self.buttons.add(3, 3, 'BATAL', click=self.cancel_click,
			bg_color=spektrum.CANCEL_BG)
		self.buttons.add(3, 4, 'TERIMA', click=self.accept_click,
			bg_color=spektrum.ACCEPT_BG) 
		if has_auto:
		...
		self.label = ui.render_text(label_text, size=spektrum.MAIN_FONT, 
			fg=spektrum.INPUT_FG, bg=spektrum.INPUT_BG)
	...
	def render(self, screen):
		...
		screen.fill(spektrum.INPUT_BG, self.input_rect)
		...
		value_label = ui.render_text('{0} {1}'.format(self.value, self.unit_text),
			size=spektrum.NUM_FONT, fg=spektrum.INPUT_FG, bg=spektrum.INPUT_BG)
...
class SettingsList(ViewBase):
	...
	def __init__(self, model, controller):
		...
		# Create buttons.
		self.buttons = ui.ButtonGrid(model.width, model.height, 4, 5)
		self.buttons.add(0, 0, centerfreq_text, colspan=4, click=self.centerfreq_click, bg_color=(60,179,113))
		self.buttons.add(0, 1, samplerate_text, colspan=4, click=self.sample_click, bg_color=(60,179,113))
		self.buttons.add(0, 2, gain_text,       colspan=4, click=self.gain_click, bg_color=(60,179,113))
		self.buttons.add(0, 3, min_text,        colspan=2, click=self.min_click, bg_color=(60,179,113))
		self.buttons.add(2, 3, max_text,        colspan=2, click=self.max_click, bg_color=(60,179,113))
		self.buttons.add(1, 4, 'KEMBALI', click=self.controller.change_to_main, bg_color=spektrum.CANCEL_BG, colspan=2)
	...
	def centerfreq_click(self, button):
		self.controller.number_dialog('FREKUENSI:', 'MHz',
...
class SpectrogramBase(ViewBase):
	...
	def render_hash(self, screen, x, size=5, padding=2):
		...
		pygame.draw.lines(screen, spektrum.TULISAN, False, 
	...
	def quit_click(self, button):
		self.controller.message_dialog('Keluar dari Program ?',
...
class WaterfallSpectrogram(SpectrogramBase):
	...
	def __init__(self, model, controller):
		...
		self.color_func = gradient_func(spektrum.WATERFALL_GRAD)
		self.waterfall = pygame.Surface((model.width, model.height))

	def clear_waterfall(self):
		self.waterfall.fill(spektrum.MAIN_BG)
...
class InstantSpectrogram(SpectrogramBase):
	...
	def render_spectrogram(self, screen):
		...
		for i in range(1, width):
			y = freqs[i]
			pygame.draw.line(screen, spektrum.INSTANT_LINE, (i-1, ylast), (i, y))
			ylast = y
