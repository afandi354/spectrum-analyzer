...
class MessageDialog(ViewBase):
	...
	def __init__(self, model, text, accept, cancel=None):
		...
		if cancel is not None:
			self.buttons.add(0, 4, 'BATAL', click=self.cancel_click, 
				bg_color=spektrum.CANCEL_BG)
		self.label = ui.render_text(text, size=spektrum.NUM_FONT,
			fg=spektrum.BUTTON_FG, bg=spektrum.MAIN_BG)
...
class SettingsList(ViewBase):
	...
	def __init__(self, model, controller):
		...
		self.buttons.add(0, 4, 'KEMBALI', click=self.controller.change_to_main)
...
class SpectrogramBase(ViewBase):
	...
	def render_hash(self, screen, x, size=5, padding=2):
		...
		pygame.draw.lines(screen, spektrum.BUTTON_FG, False, 
